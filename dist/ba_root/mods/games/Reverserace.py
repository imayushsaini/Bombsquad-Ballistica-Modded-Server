...
# Created inside BCS (Bombsquad consultancy services)
#  discord.gg/ucyaesh
# https://bombsquad.ga
#  - Reverse Race by Mr.Smoothy
...
class RaceRegion(ba.Actor):
    ...
    def __init__(self, pt: Sequence[float], index: int):
        ...
        assert isinstance(activity, ReverseRaceGame)
        self.pos = pt
        self.index = index
...
# ba_meta export game
class ReverseRaceGame(ba.TeamGameActivity[Player, Team]):
    ...
    def get_instance_description_short(self) -> Union[str, Sequence]:
        if self._laps > 1:
            return 'run ${ARG1} laps in reverse direction', self._laps
        return 'run 1 lap'
    ...
    def on_transition_in(self) -> None:
        ...
        self._regions.append(RaceRegion(pts[0], len(self._regions)))
    ...
    def spawn_player(self, player: Player) -> ba.Actor:
        ...
        spaz = self.spawn_player_spaz(
            player, position=pos, angle=270 if not self._race_started else None)
        assert spaz.node
